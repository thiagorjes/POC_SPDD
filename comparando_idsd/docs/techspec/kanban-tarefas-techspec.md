...
_Versão: 1.18 | Status: Draft | Data: 2026-09-16 | Autor: agente `/techspec`_
_PRD: docs/prd/kanban-tarefas-prd.md v1.6 — cenários congelados desde a reconfirmação da emenda v1.6_
...
> Decisões: ADR-001 a ADR-013, BDR-001, BDR-002, SDR-001 a SDR-006.
...
> ACH-05 da revisão da fatia de configuração do fluxo. SDR-006 idem: nasceu na
> emenda de 2026-09-15, que fechou ACH-02 da revisão da fatia do núcleo de
> escrita.
...
| **SDR-006** | Reconstrução total da série de tempo, parcial e declarada do estado | Torna o critério de reconstrução satisfazível; declara que `raia_id`, `titulo` e `descricao` não são curáveis pela rotina, e por quê |
| **SDR-007** | Recorte de terminais no board por instante projetado em `tarefa` | Torna RNF-011 mensurável e RN-039 aplicável na consulta; acrescenta `tornou_se_terminal_em`, reconstruível por SDR-006, e uma migration retroativa |
...
- **Envelope de RNF-009** — carga sintética com 12 meses e 5.000 tarefas, medindo
  RF-015 e RF-016.
- **Envelope de RNF-011** — o **mesmo** arnês de carga, medindo o board de RF-003
  com o recorte de RN-039 em vigor. O volume de referência é igual ao de RNF-009
  de propósito: duas cargas sintéticas divergentes envelheceriam em direções
  diferentes e a comparação entre os dois envelopes deixaria de significar algo.
  A massa precisa conter terminais **dentro e fora** da janela, senão o teste
  mede uma consulta sem recorte e passa por ausência de dado velho.
...
| RF-003 | `contracts/board-e-tarefas.md` — `GET .../board`; projeções de `data-model.md` §5; faixas sintéticas nos dois eixos da grade; recorte de RN-039 por SDR-007 | SCN-003.1 a SCN-003.4 |
...
| RNF-011 | SDR-007; `tarefa.tornou_se_terminal_em` e o índice parcial de `data-model.md` §6; recorte declarado em `contracts/board-e-tarefas.md` | Carga sintética de 5.000 tarefas — a mesma de RNF-009 —, com terminais dentro e fora da janela de 30 dias, medindo p95 da resposta completa do board |
...
| 1.16 | 2026-09-15 | agente `/techspec` | Emenda estreita fechando **ACH-02 da revisão de TASK-02.4**, em `data-model.md` §4 e §9. Nenhum requisito novo, nenhum cenário tocado, nenhuma devolução ao `/prd` consumida. Um DR novo — **SDR-006**. A §9 prometia que a rotina reescreve `tarefa`, `intervalo_tarefa` e `impedimento` **do zero**, e o log não sustenta a promessa: `raia_id` não existe em evento nenhum, `descricao` não tem evento que a registre, e `impedimento.id` é UUID gerado que o log não carrega enquanto `intervalo_tarefa` é referenciado por FK a partir dele. O que decidiu não foi a dificuldade, e sim que **registrar a raia na série de tempo desfaria RN-023** — a §5 deste mesmo documento declara que é a ausência dela ali que torna a agregação por raia impossível, de modo que ampliar o log para cumprir a promessa custaria uma regra de negócio. A promessa passa a ter dois alcances: **total** para `intervalo_tarefa`, que o catálogo de §4 determina inteiramente, e **parcial sobre linha existente** para `tarefa` e `impedimento`, com a tabela de campos reescritos e preservados escrita no lugar — a rotina não cria nem remove linha nessas duas. É o que a implementação de TASK-02.4 já fazia por imposição do esquema, sem que a spec soubesse: foi a FK de `impedimento` que a obrigou a casar linha gravada com linha reexecutada em vez de apagar e inserir. **O limite fica declarado e é real:** `titulo`, `descricao` e `raia_id` corrompidos não são curáveis pela reconstrução, e tarefa presente no log e ausente de `tarefa` é divergência reportada e não corrigida. Com isso o critério 6 deixa de ser insatisfazível por implementação correta nenhuma. Na mesma emenda, **`dados` ganha formato declarado** — um objeto por tipo de evento, chaves nomeadas, teto de 8 KiB, `descricao` explicitamente fora —, porque a reconstrução é o primeiro leitor da coluna e leitor sem formato é acoplamento ao que o escritor fez naquele dia; fecha também a metade documental de ACH-15 |
...
| 1.15 | 2026-09-15 | agente `/techspec` | Três emendas em `data-model.md` §5 e §6, fechando **ACH-03, ACH-06 e ACH-07** da revisão de TASK-02.3. Nenhum requisito novo, nenhum cenário tocado, nenhuma devolução ao `/prd`. **ACH-03 (relevante):** `impedimento` ganha `versao` e bloqueio otimista. A segunda sinalização de RN-010 não insere linha — ela lê o array de `anotacoes`, anexa e regrava o documento inteiro (SCN-009.3) —, de modo que duas sinalizações concorrentes sobre a mesma tarefa perdem uma anotação sem erro e sem rastro, que é a perda de escrita silenciosa que SDR-002 existe para impedir e que `tarefa` já evita. O índice único parcial **não** cobria o caso: ele impede dois impedimentos *abertos*, não duas escritas no mesmo. A omissão veio de analogia indevida com `intervalo_tarefa`, cuja escrita é insere-e-fecha e nunca leitura-modificação-escrita. **ACH-06:** a assimetria de chave estrangeira dentro do anel de projeção era real e não estava justificada. Passa a haver regra única e escrita: referência **dentro** do anel é FK — `impedimento.tarefa_id` e `intervalo_tarefa.tarefa_id` para `tarefa`, sem cascata, porque a tarefa não é removida fisicamente e a restrição existe para impossibilitar a linha órfã, não para propagar remoção; isso dá à rotina de reconstrução de §9 a ordem que ela pressupõe como garantia do banco e não como disciplina de quem a escreve. Referência para **fora** do anel não é FK — `projeto_id` nas três tabelas e `etapa_id` em `intervalo_tarefa` são instantâneos históricos e não vínculos vivos, e etapa arquivada ou substituída pela reconfiguração de fluxo não pode invalidar a série de tempo que já a atravessou. **ACH-07:** o índice da fila de RF-014 passa de `(condicao, projeto_id)` parcial a `(projeto_id)` parcial. Dentro de índice parcial cujo predicado fixa `condicao`, a coluna-chave `condicao` é constante em toda tupla: não discrimina, ocupa espaço em cada entrada e empurra `projeto_id` para a segunda posição |
| 1.17 | 2026-09-15 | agente `/techspec` | Emenda de uma linha fechando **ACH-07 da revisão de TASK-02.5**, em `contracts/board-e-tarefas.md`. Nenhum requisito novo, nenhum cenário tocado, nenhuma devolução ao `/prd`. O cartão passa a declarar `etapaId`. A omissão nasceu de olhar o cartão apenas de dentro do board, onde a etapa é dedutível do aninhamento — mas o cartão é o **mesmo objeto** devolvido por `POST /tarefas`, pelos movimentos, pela tomada e por `GET /tarefas/{id}`, e nesses não há lista de onde deduzir. Sem o campo, o cliente não consegue montar a `origem` que SDR-002 exige de toda escrita seguinte sem reler o board, o que transformaria um mecanismo de concorrência em ida e volta obrigatória. A suíte congelada já o exige (`CriacaoDeTarefaIT` afirma `$.etapaId`) e a implementação de TASK-02.5 já o emite: a divergência era do contrato, e ele é o que TASK-02.6 vai reusar para o board |
| 1.18 | 2026-09-16 | agente `/techspec` | Emenda absorvendo o PRD v1.6 e fechando **ACH-01, ACH-02, ACH-05 e ACH-08 da revisão técnica do board**, em `contracts/board-e-tarefas.md`, `data-model.md` §5, §6, §8 e §9 e nas Seções 2, 7 e 9. Um DR novo — **SDR-007** — e nenhuma devolução ao `/prd`: o escopo novo que esta emenda materializa **nasceu no `/prd`**, como RN-039, RNF-011 e SCN-003.4, e chega aqui já com cenário e procedência. **ACH-02 é o achado que organiza os outros:** a grade é o produto cartesiano etapa × raia, e o desenho tinha defesa num eixo só. A raia sintética existia e estava justificada — cartão que não cabe na grade some, e some **sem erro** —, e a frase inteira se aplica à etapa arquivada, onde não havia defesa alguma. Pior, a célula é alcançável por operação **permitida**: `PUT .../etapas` só recusa arquivar etapa com tarefa **ativa**, e a contagem exclui declaradamente as duas condições terminais, de modo que arquivar etapa terminal com histórico é aceito e apaga da tela todo o trabalho que passou por ela. Passa a haver faixa sintética nos dois eixos, sob **regra única** — existe quando, e só quando, há cartão que precise dela —, o que resolve junto a variação de comprimento que a suíte indexava por posição: cliente e verificação indexam por `id`. A faixa é de leitura e nunca destino de escrita, e sair dela é movimentação comum, porque a `origem` de SDR-002 carrega a `etapaId` real que o cartão sempre teve. **SDR-007 é o par de RN-039/RNF-011:** o recorte de 30 dias é aplicado na consulta, contra a coluna projetada `tarefa.tornou_se_terminal_em`, e não derivado em tempo de leitura — derivá-lo exigiria agregação por tarefa sobre `evento_tarefa`, a maior tabela do sistema, encarecendo a leitura exatamente na proporção da história que o recorte existe para esconder. A coluna é anulada na reabertura, é determinada pelo log e por isso é **curável** por SDR-006, ao contrário de `raia_id` e `descricao`; entra com índice parcial e migration retroativa de ordem 8. O recorte é da tela e nunca do registro. **ACH-01:** `acessoPorAdministracaoGlobal` passa a ser declarada no corpo do board, onde a suíte congelada já a exigia — a marca existia em `GET /v1/projetos` e no detalhe, e deduzi-la aqui obrigaria o cliente a correlacionar respostas de rotas diferentes para saber em que condição está lendo (RN-035). **ACH-05 e ACH-08 são a mesma classe:** o contrato contradizia a si mesmo sobre a forma do cartão. `permanencia` ganha `\| null`, porque permanência é intervalo **aberto** e tarefa terminal não tem nenhum — exigi-lo faria o board fabricar um `decorrido` que cresce depois da conclusão; e `assumidaEm` entra no cartão, onde sempre esteve de fato, já que as duas promessas do contrato (`POST .../tomada` e `estadoAtual` do `409`) só são coerentes se o campo for do cartão, e `tarefa.assumida_em` existe no esquema desde a primeira migration do anel |
