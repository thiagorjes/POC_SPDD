...
- **Status:** concluída (3 de 3 tentativas) — 13 achados da revisão fechados, suíte medida sem regressão (190 / 96 / 94); os sete critérios seguem sem medição por dependerem da rota de TASK-02.5 (ACH-05, destino `/tasks`)
- **Sistema:** idsd
- **Executor:** agente
- **Tentativas:** 3
...
| 2026-09-15 | tentativa 3 — medição de integração | **O Docker estava na máquina, fora do `PATH`** — mesmo padrão do Maven em `D:\CobraKai`; o CLI vive em `AppData\Local\Programs\DockerDesktop\resources\bin`, daemon 29.7.2. A suíte inteira rodou no contêiner de ADR-012: **190 testes, 96 verdes / 94 vermelhos**, contra a base 189 / 96 / 93. O teste a mais é o arquivo a mais que passou a compilar desde a tentativa 1, e ele sai vermelho pela mesma dependência das demais — **zero regressão**, e o total de verdes é idêntico ao da base. **Os sete critérios continuam sem medição, e a causa mudou de natureza.** Não é ambiente: `SeqSobConcorrenciaIT`, `ReconstrucaoDaProjecaoIT` e `ImutabilidadeDoLogIT` morrem antes da primeira asserção, em `404` de `POST /v1/projetos/{id}/tarefas`, rota que nasce em **TASK-02.5** — a falha `Expected size: 40 but was: 0` do critério 1 é ausência de escrita, não defeito do mecanismo. **ACH-05 deixa de ser pendência de ambiente e vira dependência de task**, e com isso sai do alcance desta mão: destino reatribuído para `/tasks`. **O mecanismo de ACH-01, porém, foi medido** — pelo mesmo recurso que TASK-02.3 usou quando a suíte não alcançava o critério, direto contra `postgres:16-alpine`: duas sessões tomam `pg_try_advisory_xact_lock_shared` na mesma chave e **ambas recebem `true`**, e a exclusiva sobre a mesma chave, com um compartilhado vivo, recebe `false`. É exatamente a assimetria que o desenho exige, e é a prova de que a correção de uma palavra faz o que promete; o que falta é o critério de ponta a ponta, não o mecanismo. Testada e **falsa** a hipótese de que o gate administrativo de ACH-17 teria quebrado a reconstrução: **zero `AccessDeniedException` na suíte inteira**. Os dois `BeanCreationException` são de `BroadcastMultiInstanciaIT`, que exige o perfil de broadcast e o `.env` — linha de base conhecida. |
