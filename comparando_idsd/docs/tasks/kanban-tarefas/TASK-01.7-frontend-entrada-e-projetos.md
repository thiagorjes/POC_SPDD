...
- **Status:** concluída — os seis bloqueantes da revisão fechados e medidos, e com eles ACH-12, ACH-13 e ACH-14, **corrigidos em vez de rebaixados**: corrigir dispensa o aprovador nomeado que o rebaixamento de achado de segurança exige; critério 9 declaradamente não medido, dono TASK-02.2. **ACH-16 fechado**: RNF-005 passou a ter dono nas oito tasks de frontend que estreiam tela, e a parte que cabe aqui — TL-01, TL-02 e TL-11 nas duas larguras extremas — está medida (critério 10)
- **Sistema:** idsd
- **Executor:** misto
- **Tentativas:** 5
- **Esforço:** M
...
| 10 | **RNF-005** — TL-01, TL-02 e TL-11 são funcionais a 1280 px e a 1024 px de largura, sem perda de ação nem rolagem horizontal não indicada | verificação automatizada nas duas larguras extremas, por tela (ACH-16 da revisão) |
...
| 2026-09-11 | correção ACH-16 | RNF-005 ganhou dono. A correção é de spec e não de código: o envelope passou a ser medido **por tela, na task que dá à luz a tela**, com critério de aceite nominal em cada uma das oito tasks de frontend que estreiam tela — a soma cobre as onze telas exatamente uma vez, e TASK-04.4 e TASK-08.4 ficam de fora porque não estreiam nenhuma. A alternativa era uma passagem única no fechamento do épico, que é o que reprovava por construção: tela que nasce larga demais é barata de corrigir no dia em que nasce e cara quatro épicos depois, quando o layout já foi copiado. O plano de verificação inventaria `verificacoes/largura.spec.ts` e — pela mesma ausência, que só faltava declarar — `verificacoes/acessibilidade.spec.ts`, de 12 para **14** verificações além dos cenários. O critério 10 desta task é medido aqui: TL-01, TL-02 e TL-11 a 1280 px e a 1024 px, **6/6 verdes**. A asserção tem duas metades porque uma só deixaria passar metade do requisito — rolagem horizontal do documento, com tolerância de 1 px para subpixel, e `toBeInViewport` nas ações, porque controle empurrado para fora ou colapsado a zero não produz rolagem nenhuma e some em silêncio. A medida é do documento e não de cada elemento, de propósito: rolagem dentro de região que a anuncia é permitida, e é o caso previsto para as colunas do board em TASK-02.8. Poder de falha **provado**: com `min-w-[1600px]` no título da lista, em cópia descartável, as duas larguras ficam vermelhas; revertido e verde |
| 2026-09-11 | desvio | Arquivos fora da tabela declarada, todos deliberados: `src/proxy.ts` (destino de retorno), `src/componentes/` no lugar de `frontend/components/` (imposto pela suíte congelada), `docker/keycloak/realm.json` e `docker/keycloak/tema/idsd/**` (locale pt-BR e desambiguação do rótulo do provedor), `docker/compose.yaml` (endereços internos, `NEXT_PUBLIC_APP_URL`, tema montado, healthcheck em `/entrar` porque a raiz redireciona ao provedor), `frontend/e2e/verificacoes/**` e a dependência `@axe-core/playwright`. A correção acrescentou `src/lib/auth/destino.ts` — ponto único de validação de destino, criado porque manter a regra em três lugares foi o que produziu ACH-01 — e `next.config.ts`, que a tabela não declara e sem o qual ACH-05 não fecha. A correcao dos dez achados restantes acrescentou `eslint.config.mjs`, `.prettierrc.json`, `.prettierignore` e `knip.json` (ACH-17 — o verificador de lint do DoD era inexecutavel), `src/app/error.tsx` e `src/app/not-found.tsx` (ACH-10), `src/componentes/painel-modal.tsx` (ACH-09) e `frontend/e2e/verificacoes/novo-projeto.spec.ts` (ACH-08 e ACH-09). Nenhum `.feature` nem step definition tocado |
| 2026-09-11 | correcao ACH-07 | A frase do estado de sucesso de TL-11 que prometia a marca no cartao da lista saiu. As duas sinalizacoes de RN-038 foram instituidas juntas pela emenda de 2026-09-10, e a segunda nao chega: `fluxoConfigurado` so e emitido a partir de TASK-02.2. Quem escolhe "depois" confiava num lembrete inexistente, justamente no requisito que existe porque nada no primeiro passo lembra do segundo. A tela passou a dizer o que fazer em vez de apontar para onde a pendencia apareceria, e a restauracao virou **criterio 10 de TASK-02.2**, ao lado do criterio 7 que cria o campo — mesmo tratamento de ACH-03 da revisao de TASK-01.5 |
| 2026-09-11 | correcao ACH-08 | Recusa de conteudo sem campo nomeado passou a ir para o campo da conta. Verificado no servico: so identificador nao conversivel produz `errors[]`; conta inexistente sai apenas com `detail`, e e a recusa que esta tela mais produz, porque o campo e texto livre por nao haver rota que liste contas. A atribuicao vale **so para `422`**, de proposito: `403` e as demais nao sao de conteudo de campo nenhum, e po-las ao lado do controle mandaria corrigir o que nao esta errado. Coberto por `verificacoes/novo-projeto.spec.ts`, escrito porque a revisao registrou que **nenhum teste submetia o formulario** — a acao de servidor inteira estava sem cobertura. Poder de falha **provado**: neutralizando o ramo em copia descartavel o teste fica vermelho |
| 2026-09-11 | correcao ACH-09 | O painel de TL-11 virou modal de verdade: `PainelModal` recebe o foco ao abrir e fecha por Escape, e a barra superior entrou na regiao `inert`, de onde estava fora enquanto a cortina a cobria visualmente — alcancavel por teclado e invisivel ao mouse. O alvo do link de salto migrou junto: `id="principal"` saiu do `main` inerte e passou ao painel. **Confinamento circular de foco nao foi implementado**, e isso e decisao e nao esquecimento: `inert` ja tira o que esta atras da ordem de tabulacao e da arvore de acessibilidade, e um laco proprio seria a segunda fonte da mesma regra. Nada disso e detectavel por auditoria automatica — e por isso o criterio 5 estava verde com o defeito de pe. Poder de falha **provado**: sem o `inert` o teste fica vermelho |
| 2026-09-11 | correcao ACH-10 | `error.tsx` e `not-found.tsx` criados: falha de servidor e rota inexistente deixam de cair na pagina generica do framework. As duas sao deliberadamente genericas — o `detail` do servico pode nomear recurso que a pessoa nao alcanca, e distinguir "nao existe" de "nao e seu" e exatamente o que SCN-002.3 protege. **`loading.tsx` foi escrito e depois removido, e a razao e o achado do dia**: com um shell de carregamento a rota responde `200` imediatamente e o `redirect()` ao provedor passa a ser emitido dentro do fluxo, o que aborta a navegacao de topo — medido, `page.goto` falha em `ERR_ABORTED` e a suite `e2e` caiu de 11 para 6 verdes. Em vez de um quadro em branco, quem nao tem sessao precisa chegar ao provedor; rota que redireciona antes de renderizar nao ganha shell |
| 2026-09-11 | correcao ACH-11 | Os tokens do tema escuro sairam de `globals.css` e `darkMode` saiu de `tailwind.config.ts`. Era codigo morto — caminho nenhum escreve `data-tema` — e o custo estava na medicao: a metade escura de RNF-006 ficava inauditavel, e os valores declarados **ja reprovavam** o contraste AA se um dia fossem alcancados. Instituir tema escuro e decisao de DDR, com custo proprio de contraste, e nao efeito colateral de configuracao |
| 2026-09-11 | correcao ACH-17 | O verificador de lint do DoD era inexecutavel: `"lint": "next lint"`, e `next lint` saiu no major 16 — o item nunca reprovou coisa alguma. ESLint direto em flat config (`core-web-vitals` + `typescript`), mais `prettier` e `knip`, com os tres scripts. A suite congelada entrou no `.prettierignore` com a razao escrita: reformata-la seria alterar arquivo de verificacao, que a restricao do `/implement` proibe; `e2e/verificacoes/` fica **de fora** da excecao, porque nasce aqui. Medido: `eslint` limpo, `prettier --check` limpo, `knip` limpo |
| 2026-09-11 | correcao ACH-18 | `interpretar()` em `cliente.ts`: corpo vazio vira `{}` e `JSON.parse` que falha vira `FalhaDaApi`, que e a falha tipada que todo o caminho reconhece. Antes, um `SyntaxError` cru atravessava e chegava a pagina generica do framework — e o texto recebido **nao** entra na mensagem de proposito, porque corpo de proxy carrega endereco interno |
| 2026-09-11 | correcao ACH-19 | `Object.hasOwn` na leitura do mapa de razoes de recusa em `entrar/page.tsx`: `?erro=constructor` alcancava membro de prototipo e escapava do valor padrao. E robustez e nao injecao — o valor e renderizado como texto —, mas indexar literal de objeto com entrada de terceiro e a forma, e a forma e o que se copia |
| 2026-09-11 | correcao ACH-20 | `clsx` e `tailwind-merge` declarados e nao usados sairam de producao, e `@testing-library/user-event` das de desenvolvimento. O cartao de projeto passou a consumir `permissoes`, que era campo declarado no tipo e consumido em lugar nenhum, e a rotular a participacao **pela capacidade** — "escrita" e "leitura", como o prototipo TL-02 — em vez de listar nome de papel, que e vocabulario de configuracao e nao responde a pergunta de RF-002. Sairam junto tres exportacoes sem consumidor (`ACESSO`, `redirectUri`, `Permissao`) |
| 2026-09-11 | desvio ACH-21 | Dois arquivos da tabela nunca nasceram, e a ausencia dos dois esta **certa** — o que faltava era declara-la, porque desvio nao declarado e indistinguivel de esquecimento na proxima leitura. O cliente de dados de navegador nao existe porque nenhum componente de tela alcanca o gateway: toda leitura e Server Component e toda escrita e Server Action, e `import 'server-only'` no cliente REST torna a violacao erro de compilacao. O cliente de mensageria nao existe porque nada nesta task consome evento — o tempo real nasce em EPIC-02, e criar o cliente aqui seria codigo sem consumidor, que e exatamente o que ACH-20 acabou de remover |
| 2026-09-11 | medicao | Depois das dez correcoes: `e2e` **34 testes, 20 verdes / 14 vermelhos**, contra a linha de base de 31 (17/14) — os 3 novos sao `verificacoes/novo-projeto.spec.ts` e os 14 vermelhos sao identicos, todos de EPIC-02 em diante. `tsc --noEmit` com as duas falhas pre-existentes de EPIC-03 e nada mais; Jest identico a linha de base; `eslint`, `prettier --check` e `knip` limpos. Nenhum `.feature` nem step definition tocado |
