...
3. **Códigos ad-hoc** — lançados via `BusinessException(errorCode, message)` direto, sem subclasse dedicada (casos pontuais de uma única Service):
   | errorCode | HTTP | Origem |
   |---|---|---|
   | `PERIODO_INVALIDO` | 422 | `DashboardService.obter` — `fim <= inicio` ou janela > 365 dias |
   | `TOGGLE_DESCONHECIDO` | 422 | `ProjetoService.atualizarToggles` — chave fora de `ChaveToggle` |
   | `PAPEL_PROTEGIDO` | 422 | `UsuarioProjetoPapelService.associar` — papel global/protegido (RN-006) |
4. **Uso**: lançadas exclusivamente pela Service Layer; Controller nunca lança regra de negócio.
...
**Padrões de controller**: `@RestController`, `@RequestMapping("/api/...")`, `@Validated`, `@RequestBody @Valid`, retorno `ResponseEntity<T>` com `201 + Location` na criação e `204` em exclusão. O Controller **não** carrega `@ExigePermissao`: a autorização vive na Service Layer (A-15), garantindo que `projetoId` seja sempre derivado do recurso (RNF-003).
...
1. **`backend/Dockerfile`**: multi-stage `maven:3.9-eclipse-temurin-25` (com `mvn dependency:go-offline` antes de copiar o código) → `eclipse-temurin:25-jre`, executando como usuário não-root. O `package` **não** roda com `-o`: o `go-offline` não pré-resolve os intervalos de versão do Cucumber, que exigem consultar o metadata remoto.
   - O `release` do compilador vem de `${java.version}`. Em máquina de desenvolvimento sem JDK 25 instalado, `-Dmaven.compiler.release=21` **não** tem efeito (o pom usa a propriedade, não o default do plugin): use `mvn compile -Djava.version=21` para validação local rápida. A validação canônica do target 25 é o `docker compose build`, que roda sobre `temurin-25`.
   - O driver `org.postgresql:postgresql` fica em escopo **compile**, não `runtime`: o `BoardEventLoop` importa `PGConnection`/`PGNotification` para o `LISTEN/NOTIFY` (ADR-004).
2. **`frontend/Dockerfile`**: multi-stage `node:20-alpine` (`npm ci && npm run build`) → runtime `output: standalone`. Requer `frontend/public/` versionado (ainda que vazio), senão o `COPY --from=build /app/public` falha.
   - **Configuração do cliente é lida em runtime, não em build**: o Next inlina `process.env.NEXT_PUBLIC_*` no bundle durante o `npm run build`, que roda sem `.env`. Ler essas variáveis direto do bundle no navegador entrega string vazia — o `keycloak-js` recebe `url`/`realm` vazios e a página quebra com exceção client-side. Passar as URLs como `build args` também é proibido: assaria endereço de ambiente na imagem, contra ADR-008.
   - Solução: o `app/layout.tsx` (Server Component) lê `process.env` **a cada request** e injeta as chaves públicas em `window.__KANBAN_ENV__` via `<script>`; `lib/config.ts` centraliza a leitura e é a **única** origem de configuração para `lib/auth`, `lib/api` e `lib/stomp`. Uma imagem serve qualquer ambiente.
   - Só entram nesse objeto valores **públicos por natureza** (URLs, realm, clientId). Segredo nunca é injetado no HTML — em client público não há segredo a proteger.
   - O layout raiz precisa de `export const dynamic = 'force-dynamic'`, senão o Next pré-renderiza o script na build e congela os valores.
...
4. **`infra/keycloak/kanban-realm.json`**: realm importado no boot do Keycloak (`start-dev --import-realm`), sem o qual não existe issuer e o login falha por ADR-006. Conteúdo mínimo:
   ...
   - Client público `kanban-frontend`: `publicClient=true`, `standardFlowEnabled=true`, `directAccessGrantsEnabled=true` (permite obter token via `curl` para teste manual de API), PKCE `S256` obrigatório, `redirectUris`/`webOrigins` derivados de `${FRONTEND_PUBLIC_URL}` (nunca porta literal, senão trocar `FRONTEND_PORT` quebra o login com `invalid_redirect_uri`). **Sem client secret** — segredo em client público é falso controle.
   - O importador do Keycloak é estrito: campo desconhecido no JSON aborta o boot. Comentários só cabem em campos reais da representação (ex.: `description` do client).
...
| A-15 | Onde vive `@ExigePermissao` (Controller vs. Service) | **Service Layer** — fonte única de autorização. O Controller apenas delega; assim toda entrada (REST, STOMP, futuro job) passa pelo mesmo guard e `projetoId` é sempre derivado do recurso (RNF-003). O teste ArchUnit da §11 deve verificar a anotação **no método de Service**, não no handler |
| A-16 | Autorização quando a Service depende de contexto (ex.: `mover`, `atribuir`) | `PermissaoGuard` imperativo dentro do método, na mesma ordem fixa da RN-015 (permissão → toggle → projeto ativo) |
...
- Teste de arquitetura ArchUnit bloqueia: método de escrita da Service Layer sem `@ExigePermissao` nem chamada a `PermissaoGuard` (A-15/A-16), Service acoplada a adapter de evento, campo boolean com nomenclatura que quebra JavaBeans.
